...
class TextMessage(QLabel):
    ...
    def setText(self, p_str):
        super(TextMessage, self).setText(p_str)
        font_metrics = QFontMetrics(self.font())
        rect = font_metrics.boundingRect(p_str)
        self.setMaximumWidth(rect.width() + 30)
        self.heightSingal.emit(rect.height() + 30)
...
class Avatar(QLabel):
    def __init__(self, avatar, parent=None):
        ...
        if isinstance(avatar, str):
            # 使用 Qt.SmoothTransformation 来确保图像在缩放时更加清晰
            self.setPixmap(QPixmap(avatar).scaled(45, 45, Qt.KeepAspectRatio, Qt.SmoothTransformation))
            self.image_path = avatar
        elif isinstance(avatar, QPixmap):
            self.setPixmap(avatar.scaled(45, 45, Qt.KeepAspectRatio, Qt.SmoothTransformation))
...
class BubbleMessage(QWidget):
    def __init__(self, str_content, avatar='', Type=1, is_send=False, parent=None):
        super().__init__(parent)
        self.isSend = is_send
        # self.set
        self.setStyleSheet(
...
class ChatWidget(QWidget):
    def __init__(self):
        ...
        # 设置内容部件的最小尺寸为0，以防止不必要的拉伸
        self.scrollAreaWidgetContents.setMinimumSize(0, 0)
    ...
    def set_last_item_text(self, text):
        self.layout0.itemAt(self.layout0.count() - 2).widget().setText(text)
